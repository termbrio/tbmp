...
5. A successful explicit Team stop abandons an incomplete `create-once` submission when no real provider conversation identity was recorded. The next start creates again. Do not delete the Server database or reset the assistant merely to recover this normal stopped attempt; if the current product still reports `create-submitted` after a successful stop, report the exact status as a product defect instead of looping start.
