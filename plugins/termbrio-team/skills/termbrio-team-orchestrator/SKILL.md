...
1. Run zero-side-effect semantic preflight with `tb team validate FILE --resolve --json`. Require every member to use exactly one binding: `session.id` or `session.ref` must resolve an existing session, while `session.name` explicitly permits create/reuse. Never place a UUID in `session.ref`.
...
5. Inspect `tb team status TEAM --json` and the initialization result. Do not infer readiness from quiet terminal output.
...
Return the team name, saved revision, initialization operation id/state, per-member readiness, and the exact visible-start command. Do not include prompt bodies, member identity values, or secret environment values.
