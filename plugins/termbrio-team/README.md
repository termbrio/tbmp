...
Codex and Claude lifecycle hooks report provider-neutral member activity through the shared synchronous `hooks/hooks.json` manifest. The bounded two-second commands cover session start/end, prompt submission, all tool-use transitions, permission/input requests, actionable notifications, and completion. Hooks call the package-local lightweight `tbhookemit` transport instead of starting the full `tb` command host for every event. This lets later tool activity clear stale question or permission evidence without putting CLI startup, dependency injection, or database work on the provider's critical path. A single cross-provider manifest avoids duplicate Codex registration and unsupported asynchronous-hook warnings. Hook transport failure never fails the provider. Termbrio retains bounded provider metadata and summaries rather than raw hook stdin.
